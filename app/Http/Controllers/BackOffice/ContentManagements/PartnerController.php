...
use App\Http\Requests\BackOffice\ContentManagements\ContentManagementPartnerStoreRequest;
...
class PartnerController extends Controller
{
    ...
    {
        $data = $this->contentManagementRepository->get(ContentManagementCodeKey::PARTNER, $request->trade_name, true);
        return $this->success($data);
    }

    public function store(ContentManagementPartnerStoreRequest $request)
    {
        try {
            DB::beginTransaction();
            $data = $this->contentManagementRepository->updatePartner($request);
            DB::commit();
            return $this->success($data);
